...
**커버리지 갭(정직하게 미커버로 남긴 것, 2026-09-10 5차 라운드 갱신)**: `SpanResolution.FAILED`(위치 특정 불가 전체 실패), `DeletionReport` 전체, `ReadoutRun.outcome=PARTIAL`, `TimeResolution.status=UNKNOWN`(시각을 전혀 알 수 없는 경우), `RequirementReport.overall=BLOCK`, `info_state=INFO_AI_ESTIMATED`, `location.address`/`place_name`(reverse-geocode artifact 필요), `CaseView.progress[].state=PARTIAL`(신규 등재, `AnalysisRun.outcome=PARTIAL` 투영 — 어느 시나리오도 `AnalysisRun.outcome=PARTIAL` 자체가 없어 fixture 데모 불가, `05` §12 유소연-⑧ 참고), `scenario_plate_reread_001`의 재판독 성공 후 상태(`exec_p001_plate_reread`가 현재 `QUEUED`·`produced=[]`로 멈춰 있음 — 완료 후 판독값·`review_needed` 전환은 다음 라운드에 별도 `case_rev`/시나리오로 추가 예정, 이슈 #22 B-3 부가질문 회신 근거 `docs/modules/case/decisions/eval-round2-ground-truth-and-usage.md`). ~~`SpanResolution.PARTIAL`~~·~~`ReadoutRun.outcome=FAILED`/`JobExecution.status=FAILED`~~·~~`JobExecution.STALE`/재시도~~·~~`AnalysisScope.TIMELINE_RELATIVE`/`timeline_status=USABLE_RELATIVE_ONLY`~~·~~GPS Observation(있음/없음)~~은 이번 라운드(`scenario_infra_failure_001`·`scenario_relative_rebase_001`·GPS fixture)로 해소했다. (`VisualEvidence.verification`의 `UNCERTAIN`은 2026-09-09 시나리오 분리로 `unknown_abstain_partial`이 커버하게 되어 이 목록에서 제외했다 — `NOT_OBSERVED`는 여전히 미커버.) 7개 시나리오 안에 억지로 욱여넣기보다 갭으로 남기고 기록하는 편이 "절대 조용히 결정하지 않는다" 원칙에 맞다고 판단했다. 남은 갭 중 `scenario_blocked_001`(BLOCK/INFO_AI_ESTIMATED)은 `05_mock_deep_review_report.md` §10·§13(row 9)에 근거와 함께 정리돼 있다.
