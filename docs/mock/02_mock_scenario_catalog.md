...
- (v3 추가) 재판독이 실제로 **성공**해 `abstained=false`로 전환되고, 그 결과가 `EvidenceRecord`의 immutable snapshot 원칙(§10 Invariants 2 — 「동일 Record를 mutate하지 않는다」)에 따라 **새 revision**(`supersedes_ref`로 연결)으로 반영되며, `RequirementReport(EVIDENCE)`가 `UNKNOWN`→`PASS`로 넘어가는 것까지 한 시나리오 안에서 끝까지 보여준다 — `EvidenceNeeds`가 발주한 재작업이 실제로 완결되는 경로를 이 pack에서 처음 fixture화한다.

**시작 조건**: 영상 1개, 신호 위반 장면은 화면에서 명확히 관찰됨(사건 유형 확정). 번호판만 프레임 간 인식이 갈림.

**예상 흐름**: `COARSE_SEARCH` → 후보 1건(`ranking_score 0.83`, `uncertainties=[]`) → `VisualEvidence.verification=OBSERVED`·`visual_event_type=SIGNAL` → `PLATE_READ` 성공하지만 `target_association=ASSOCIATED`인 채로 `abstained=true`(프레임 간 OCR 불일치) → `OVERLAY_TIME_READ` 정상 성공, `TimeResolution.status=OK` → `evidence.assemble()`이 `EvidenceRecord`(`ev_p001`)에서 `vehicle_number` 필드를 아예 비움, `event`·`occurred_at`은 전부 확정 → `EvidenceNeeds`가 `PLATE_REREAD` 요청 → `case`가 `force_rerun=true`로 새 Job 자동 발주(`case_rev:3` 스냅샷에서 `QUEUED`, `occurred_at`·`visual_event_type`은 리셋되지 않음) → `RequirementReport(EVIDENCE).overall=UNKNOWN`(원인은 `vehicle_number` 하나로 명확히 국한) → **(v3)** 재판독 `JobExecution`이 `SUCCEEDED`로 종료(추가 프레임 `fr_p001_plate3`까지 샘플링해 프레임 간 합의 도달) → 새 `PlateReadout`(`readout_p001_plate_reread`, `abstained=false`, `consensus.text="17나2867"`) → `evidence`가 `EvidenceRecord ev_p001_v2`(`supersedes_ref=ev_p001`, `vehicle_number` 필드 신규 채움)와 `RequirementReport req_p001_evidence_v2`(`overall=PASS`)를 생성 → `CaseView case_rev:4`(`running_jobs=[]`, `requirements_evidence.readiness=PASS`, `plate_display` 확정 표시로 전환, `evidence.plate_abstained` notice 소거).

**기대 결과**: 재판독 성공 이후에도 `ReportPackage`는 여전히 생성되지 않는다(§8.1 ready-only 규칙) — `EVIDENCE` 게이트 통과와 `PACKAGE_READY`(`FINAL_PACKAGE` scope report + `ReportPackage`)는 서로 다른 gate이고, 이 시나리오는 `REPORT_VIDEO_EXPORT` Job을 발주하지 않기 때문이다(`contract-job-record-case-view.md` §10 Invariants 9). 그래서 `case_rev:4`에서도 `stage`는 여전히 `EVIDENCE_REVIEW`이고 `CaseView.requirements_package=null`·`package=null`이며, 이는 이 pack에서 「`EVIDENCE_SUFFICIENT=true`이지만 `READY`는 아닌」 상태를 실증하는 유일한 스냅샷이다. `CaseView.evidence.event_time_display`는 처음부터 `INFO_SOURCE_VERIFIED`로 확정 표시됐고 재판독으로 변하지 않는다.
...
>
> **2026-09-10 v4 수정 (4차 통합, 이슈 #33 A-5 답변 — v3 이월 버그 2건 수정).** ① `ev_p001_v2.selection_rev`가 v3 작업 중 `correction_rerun_001`의 패턴을 잘못 유추해 `1→2`로 올라가 있던 것을 `1`로 되돌렸다 — 재판독은 새 후보를 다시 선택한 것이 아니라 같은 후보 위에서 번호판만 다시 읽은 것이라, 이슈 #33에서 `plate_reread_001`에 한정해 이 되돌림을 명시적으로 요청했다. ~~(`correction_rerun_001`의 `selection_rev` 1→2 bump는 후보 재선택이 실제로 있었던 정당한 사례라 그대로 뒀다)~~ → **정정 (2026-09-11, 이슈 #39 Required-3, 김준영 `evidence` 지적).** `correction_rerun_001`도 같은 버그였다 — 사용자의 사건시각 정정은 후보 재선택이 아니므로 `selection_rev`가 오르면 안 됐다. `cr_r001_time.selection_rev`와 `ev_r001_v2.selection_rev`를 모두 `1`로 되돌리고, `candidates[0].at`/`at_provenance`도 correction 전(rev2)과 동일하게 유지하도록 `case/scenario_correction_rerun_001.json` rev3을 고쳤다 — candidate 자체는 재선택되지 않았으니 그 timeline 위치가 correction으로 바뀔 이유가 없다. 자세한 내용은 아래 `scenario_correction_rerun_001` 절 참고. ② 재판독 readout(`readout_p001_plate_reread`)이 `frame_results[]`에서 원 판독과 같은 `crop_p001_001`/`crop_p001_002`를 재사용하고 있던 것이 `crop_ref` 동일성 규칙(`frame_ref`·`bbox`·`extraction_params=input_ref.source_profile` 중 하나라도 다르면 다른 `crop_ref`, 이슈 #31 A-3)을 위반해 `crop_p001_004`/`crop_p001_005`로 새 id를 부여했다 — `source_profile`이 `readout-native`→`readout-native-hires`로 바뀌었기 때문이다. 원 판독의 `crop_p001_001`/`crop_p001_002`와 새로 추가됐던 `crop_p001_003`은 그대로 뒀다.
...
**예상 흐름**: `COARSE_SEARCH` → 후보 선택 → `PLATE_READ` 성공(비-abstain) → `OVERLAY_TIME_READ` 실행되나 오버레이 부재로 `NOT_APPLICABLE` → `TimeResolution v1`(`NEEDS_REVIEW`, `BASE_PLUS_OFFSET`) → `EvidenceRecord v1`(`occurred_at.resolution_status=NEEDS_REVIEW`) → 사용자가 정정 제출(`CorrectionRecord cr_r001_time`, `kind=EVENT_TIME_MANUAL`, case fixture에 정식 필드 스키마로 존재) → `TimeResolution v2`(`status=OK`, `verification=AGREED`, `user_corrected=true`, `computation.mode=USER_OVERRIDE`, `supersedes_ref=v1`) → `EvidenceRecord v2`(`occurred_at`만 갱신, `vehicle_number`는 v1과 완전 동일) → `RequirementReport v1(WARN)`→`v2(PASS)`. 후보 재선택은 일어나지 않으므로 `CaseView.candidates[0].at`/`at_provenance`와 `EvidenceRecord`/`CorrectionRecord`의 `selection_rev`(둘 다 `1`)는 correction 전후로 그대로다 — 바뀌는 것은 `evidence.event_time_display`/`occurred_at`뿐이다(이슈 #39 Required-3).

**기대 결과**: `EvidenceRecord v2.vehicle_number`가 `EvidenceRecord v1.vehicle_number`와 값·source·support_refs·needs_review 전부 동일. `CaseView.evidence.user_edited=true`이지만 `plate_display`는 그대로. `CaseView.candidates[0].at`/`at_provenance`도 rev2·rev3 전부 동일(`recording.filename_time`, `2026-08-27T13:15:30+09:00`) — candidate의 timeline 위치는 correction으로 바뀌지 않는다.
...
> **2026-09-10 v4 추가 (4차 통합, 이슈 #33 — `CorrectionRecord` Draft→Final 승격 후속).** `contract-correction-record.md`가 `correction-record/v1.1`(Final — Accepted)로 승격되면서, 그동안 opaque ref로만 존재하던 사용자 정정 제출을 실제 `correction_records[]` fixture로 처음 구체화했다 — `cr_r001_time`(`case_id:"case_r001"`, `kind:"EVENT_TIME_MANUAL"`, `target_field:"occurred_at"`, `previous_value`는 filename+offset 추정치, `new_value`는 사용자가 정정한 시각, `supersedes_ref:null`, `corrected_at`은 `req_r001_evidence_v1` 평가와 `v2` 평가 사이 시점). `TimeResolution v2`/`EvidenceRecord v2`가 참조하는 `{kind:"correction_record", ref:"cr_r001_time"}`는 이제 실제로 정의된 대상을 가리킨다. 같은 시나리오의 `event.safety_report_type.value`도 두 `EvidenceRecord`(v1·v2) 모두 한글 라벨 placeholder에서 `MOTORCYCLE_VIOLATION`으로 정규화했다. ~~`selection_rev:2`~~ → **정정 (2026-09-11, 이슈 #39 Required-3).** 최초 배포분은 `cr_r001_time`과 `ev_r001_v2` 모두 `selection_rev:2`였으나, 이 correction은 candidate를 재선택한 적이 없는 순수 시각 정정이라 `selection_rev`는 correction 발생 시점 값(`1`)으로 고정돼야 한다(`contract-correction-record.md` §4 — "수정 횟수 카운터가 아니라 그 수정이 일어난 candidate 선택 context"). 둘 다 `1`로 되돌리고, `case`의 rev3 `candidates[0].at`/`at_provenance`도 rev2와 동일한 값(`recording.filename_time`)으로 되돌렸다 — correction 전에는 이 두 필드도 `case.user_correction`으로 잘못 앞당겨 바뀌어 있었다. 이 시나리오를 owner인 case가 직접 만들었기 때문에 처음부터 이 버그를 안고 있었고, evidence(김준영)가 이슈 #39 검수에서 지적했다. `data/mock/validate_mock_pack.py`에 이 정합을 지키는 검사(§8-1)를 새로 추가했다.
...
**기대 결과**: `common/scenario_infra_failure_001.json`에 두 개의 `JobExecution`(attempt 1 STALE, attempt 2 FAILED)이 동일 `job_id`·서로 다른 `execution_id`로 존재. `readout/scenario_infra_failure_001.json`은 `readout_runs`에 `outcome=FAILED` 항목 1개, `plate_readouts=[]`. `case/scenario_infra_failure_001.json`의 rev2 `notices`에 `severity=ERROR`·`blocking=true`인 `readout.plate_read_failed`와, `severity=INFO`·`blocking=false`인 `readout.overlay_presence_undetermined`가 함께 존재(rev1부터 후자만 먼저 등장).
...
>
> **2026-09-10 v3 후속 추가 (PR #27 `docs(readout): failure taxonomy 확정` 리뷰 요청 반영, 신유민→유소연).** `failure-taxonomy.md`가 확정한 "실패가 아닌 상태" 2행(overlay 「없음」→`NOT_APPLICABLE` / 「확인 못함」→`UNKNOWN`) 중 「없음」쪽 notice(`readout.overlay_not_present`)는 `scenario_unknown_abstain_partial_001`에 이미 있었지만, 이 시나리오의 `OVERLAY_TIME_READ`가 표현하는 「확인 못함」(`UNKNOWN`, `reason.code=readout.overlay.presence_undetermined`)쪽은 대응 notice가 빠져 있었다 — PR #27 리뷰에서 지적된 그대로다. `case_views[0].notices`(rev1)·`case_views[1].notices`(rev2)에 `{code:"readout.overlay_presence_undetermined", severity:"INFO", blocking:false, message_key:"notice.overlay_presence_undetermined", actions:[]}`를 추가해 두 "실패가 아닌 상태" 모두 notice로 대칭 표현되도록 맞췄다 — `readout.plate_read_failed`(ERROR, blocking)와 나란히 있어도 `overlay_presence_undetermined`는 blocking하지 않는 INFO로 두어 "이건 읽기 실패가 아니라 원래 확인이 안 되는 상태"라는 taxonomy 문서의 전제(`progress[overlay_time_read].state`는 rev1부터 이미 `DONE`, `FAILED`로 내려가지 않음)와 일치시켰다.
>
> **2026-09-12 v5 추가 (`04_mock_validation_report.md` Should-1 비차단 후속 2건 처리, 유소연).** 계약에는 등재됐지만 이 pack에 실사용 fixture가 없던 두 갈래를 이 시나리오에 additive하게 이어 붙였다 — `evidence`가 전 revision에 걸쳐 `null`이라 다운스트림(`EvidenceRecord`/`TimeResolution`/`RequirementReport`) 리스크가 없는 시나리오이기 때문이다. `job_x001_plate_reread`(`force_rerun=true`, case_rev 3)의 `JobExecution`을 `RUNNING→CANCELLED`(`job-execution/v1.1` 이슈 #33 A-2 전이)로 종료시켜 `ended_at` 설정·`produced=[]`·`UsageRecord(run_ref=null, run_ref_reason=RUN_NOT_PRODUCED)`를 남기고, `CaseView rev3`에서 `progress[plate_read].state`를 `FAILED`가 아닌 `PARTIAL`로(`case-view/v1.3` §7/§13 — 새 enum 값 없이 `CANCELLED→PARTIAL` 흡수) 옮기면서 기존 blocking `readout.plate_read_failed` notice를 `severity=INFO`·`blocking=false`인 `case.plate_read_cancelled`(action `RETRY_PLATE_READ` 유지)로 교체했다. 이어 `job_x001_overlay_reread`(case_rev 4)의 `OVERLAY_TIME_READ`는 `outcome=SUCCEEDED`이면서 `observation.status=UNKNOWN`·`reason.code=readout.overlay.ocr_failed`(`failure-taxonomy.md`의 `OVERLAY_VALIDATION` — run 실패가 아니다)로, `validation.format_ok=false`인 3개 `samples`(파싱 불가한 `raw_text`, `parsed_at=null`)와 함께 처음 실사용했고, `CaseView rev4`의 notices에서 `readout.overlay_presence_undetermined`를 `readout.overlay_ocr_failed`(INFO, non-blocking, actions=[])로 교체했다(같은 UNKNOWN 행의 두 reason.code를 문서가 "합치지 않는다"고 못박은 대로, notice도 갈아 끼우되 재사용하지 않았다). 재시도이므로 readout 쪽엔 `supersedes_ref`를 두지 않고(§99/§98 — readout retry는 항상 새 `run_id`, "현재본" 판단은 case 책임) 전부 새 ID로만 추가했다. 두 신규 `OverlayTimeReadout` sample의 `frame_ref`가 가리키는 `FrameRef` 3건도 `recording/scenario_infra_failure_001.json`에 추가했다. 이 작업 과정에서 `validate_mock_pack.py`의 `CLOSED_ENUMS`가 `job_executions.status`에 `CANCELLED`를, `progress[].state`에 `PARTIAL`을 각각 반영하지 못하고 있던 것(둘 다 이전 라운드에서 계약에는 추가됐으나 validator가 갱신되지 않았던 잔여 버그)도 함께 고쳤다.
