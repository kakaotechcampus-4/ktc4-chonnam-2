...
→ search: AnalysisRun(run_h001_search, CANDIDATE_SEARCH, SUCCEEDED) → CandidateEvent(cand_h001, rank1)
→ case: JobRecord(job_h001_visual) 발주 → common/runtime: JobExecution(exec_h001_visual, SUCCEEDED)
→ search: AnalysisRun(run_h001_visual, VISUAL_VERIFY, SUCCEEDED) → VisualEvidence(ve_h001, OBSERVED)
...
- Candidate Search `AnalysisRun.outcome=SUCCEEDED`이고 `CandidateEvent`가 1개 이상 존재한다(Candidate 0개 정상도 별도 시나리오 대상이나 이번엔 1개로 진행)
- Fine 관찰은 별도 `VISUAL_VERIFY` AnalysisRun으로 기록되고 `VisualEvidence.run_id`가 이 Run을 참조한다
...
→ search: AnalysisRun(run_p001_search, CANDIDATE_SEARCH, PARTIAL, issue 1건) → CandidateEvent(cand_p001, rank1, ranking_score 낮음)
→ case: JobRecord(job_p001_visual) 발주 → common/runtime: JobExecution(exec_p001_visual, SUCCEEDED)
→ search: AnalysisRun(run_p001_visual, VISUAL_VERIFY, SUCCEEDED) → VisualEvidence(ve_p001, OBSERVED이지만 target AMBIGUOUS)
...
- Candidate Search `AnalysisRun.outcome=PARTIAL`이고 `issues.length>=1`이다(불변조건 §6-1 7번)
- Visual Verify는 별도 `AnalysisRun.outcome=SUCCEEDED`이며 target association만 `AMBIGUOUS`다
