...
- **Search Fine Job kind 사용.** `AnalysisRun.operation=VISUAL_VERIFY`와 "public capability invocation마다 새 AnalysisRun"이라는 Final Contract를 지키기 위해 Mock은 Candidate Search와 별도 `JobRecord.kind=VISUAL_VERIFY`를 사용한다. `JobRecord.kind`는 닫힌 enum이 아니므로 허용되는 확장으로 해석했지만, 계약의 "확인된 값" 목록에는 아직 없으므로 case/search Owner가 선호하는 Job kind 이름은 확인이 필요하다. 이는 `AnalysisRun.operation`을 새로 만든 것이 아니라 이미 확정된 값을 Job Intent에 연결한 것이다.
