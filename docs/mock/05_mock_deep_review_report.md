...
> - ⏳ **여전히 열려 있음(이번 라운드도 범위 밖)** — `scenario_blocked_001` · `location.address` reverse-geocode artifact · `plate_reread_001`의 재판독 성공 후 상태(다음 라운드 예정, `04` §1 커버리지 갭 등재) · `progress[].state="중단"`(대응 `JobExecution.status` 값 자체가 없어 김준영 확인 선행 필요, `CONTRACT_CONFLICTS.md` 항목 9)
> - ✅ **같은 날 후속 반영** — §12 유소연 ⑦(원래 갭 아니었음, `progress[]`는 AI job 전용이라 사용자 입력 단계는 대응 값이 없는 게 맞다는 근거를 계약에 명문화) · ⑧ 절반(`progress[].state=PARTIAL` 등재) · `timeline_revision`/`stale_revision`/`stale_revision_label_key`를 나머지 5개 시나리오(`empty`는 `candidates=[]`라 제외)에 backfill
