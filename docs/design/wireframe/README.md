...
![후보 검토](08-candidates.png)

**전체 흐름 STEP 04–05의 뒷부분**이고, 진행 화면에서 「다른 장면이었나요?」로도 들어온다.

찾은 후보를 **세로로 쌓지 않고 나란히 놓는다.** 세로로 쌓으면 스크롤하며 기억에 의존해 비교하게 된다.

카드마다 시각 · AI가 관찰한 문장 · 상황 확인 · 시각 출처가 붙고, 위에 **사용자가 말한 기억 단서**를 한 번 띄운다. 짝은 위치로만 만들고 **맞는지 틀리는지 화면이 판정하지 않는다** — 그 근거가 계약에 없다.

카드 번호는 위쪽 시간축 마커 번호와 같다. 목록과 시간축을 눈으로 잇지 못하면 비교가 성립하지 않는다. 5초 안에 겹친 마커는 개수로 접어 두고 눌러야 펼친다.

썸네일 자리는 접지 않고 「미리보기 준비 중」을 적는다 — 자리를 비우면 값이 없는 것인지 기능이 없는 것인지 구분되지 않는다.

> **이 화면만 코드가 올라가 있다** — [PR #124](https://github.com/kakaotechcampus-4/ktc4-chonnam-2/pull/124) · [PR #125](https://github.com/kakaotechcampus-4/ktc4-chonnam-2/pull/125)

## 9. 둘만 견주기

![둘만 견주기](09-compare.png)

「여러 개를 훑는 것」과 「둘만 자세히 견주는 것」은 다른 동작이라 모드를 나눈다. 탭으로 전환한다.

같은 항목을 **같은 행에 맞춰** 놓는다. 행이 어긋나면 비교가 성립하지 않는다. 항목이 많을 때를 위해 「다른 점만 보기」를 둔다.

아래 선반에 상위 후보를 남겨 스크롤해도 다른 후보로 바로 건너뛸 수 있게 한다.

## 10. 어떻게 정했는지

![어떻게 정했는지](10-evidence.png)

진행 화면의 「어떻게 정했는지」로 들어온다. **값은 이미 정해져 있고, 여기서는 근거를 보고 틀렸으면 되돌린다** — 항목마다 묻지 않는다.

발생 시각처럼 출처가 둘로 갈릴 때 **양쪽을 나란히 놓고 어느 쪽을 왜 썼는지 적는다.** 합치거나 평균내지 않는다.

발생 장소를 못 구했으면 「알 수 없음」과 함께 **왜 모르는지와 어디서 채우면 되는지**를 적는다 — 여기서 묻지 않고 안전신문고로 넘긴다.

판정값을 그대로 내보내지 않는다. `PASS`가 아니라 「충족」, 판정 자체가 없는 것은 「확인 전」으로 따로 적는다.

## 11. 번호판 확인

![번호판 확인](11-plate.png)

원본 프레임에 번호판 영역을 표시해 **확대해도 어디를 보고 있는지 잃지 않게** 한다. 확정하지 못한 자리는 오렌지로 남기고 그럴듯한 값으로 채우지 않는다.

> **화면 전체가 계약 대기다.** `CaseView`에 번호판 프레임을 가리키는 필드가 없다 — `thumb_ref`·`preview_ref`는 사건 대표 썸네일이고 `crop_ref`는 이름표일 뿐이다 → [#47](https://github.com/kakaotechcampus-4/ktc4-chonnam-2/issues/47)
>
> 계약에 넣을 때 `plate_preview_ref`를 **그 값 하나로 이미지를 가져올 수 있게** 정의해야 한다. 이름표만 받으면 지금과 똑같이 막힌다.

## 12. 결과 없음

![결과 없음](12-no-result.png)

후보 0건. **실패가 아니라 정상 결과 중 하나다.** 「영상은 정상적으로 모두 확인했습니다」를 먼저 말한다.

조건을 자동으로 넓히지 않는다 — 자동으로 넓히고 결과를 섞으면 왜 이 후보가 나왔는지 추적할 수 없다. 「아침 7시쯤 → 6~9시로 넓히기」처럼 **무엇을 바꾸는지 보여주고** 누르게 한다.

## 13. 신고자료 — 템플릿으로 제출

![신고자료 템플릿](13-handoff.png)
...
| `08-candidates.png` | 후보 검토 | `03_Main_Flow` 내부 |
| `09-compare.png` | 둘만 견주기 | `03_Main_Flow` 내부 |
| `10-evidence.png` | 어떻게 정했는지 | `03_Main_Flow` 내부 |
| `11-plate.png` | 번호판 확인 | `03_Main_Flow` 내부 |
| `12-no-result.png` | 결과 없음 | `03_Main_Flow` 내부 |
| `13-handoff.png` | 신고자료 | `09-handoff · 화면` (상단 시트) |
