...
>
> **종결 재확인(2026-09-20):** #97이 P/R 공용 evidence fixture와 `CaseView.requirements_evidence`를 active catalog와 동기화했고, Web Consumer도 `WARN`을 blocking/UNKNOWN으로 오해하지 않는다고 #86에서 확인했다. 따라서 마지막 미표시였던 RequirementReport 항목도 현재 정본인 **H PASS/PASS · U WARN/WARN · P UNKNOWN→WARN · R WARN→WARN**으로 정정해 표시한다.
>
> #114로 real Happy 경로에 `case.hints.location`이 전달돼 `EVIDENCE=PASS`가 되었지만, real `FINAL_PACKAGE`는 `situation_response`·최종 REPORT_VIDEO 관찰 I4가 아직 연결되지 않아 별도 Runtime 후속이다. #115는 I4를 “이미 존재하는 readout Producer 배선”이 아니라 **REPORT_VIDEO 재관찰 input 계약/capability 확장 → case → evidence 배선**으로 정정했다.
>
> **읽는 법:** 아래 체크박스는 evidence의 **baseline/Contract/Consumer 접합 1차 완료 조건**에 대한 증빙이다. 문서 최초 작성일(2026-09-12)에 적은 역사적 설명과 당시 수치는 보존하되, 현재 상태 판정은 이 종결 재확인과 [`first-completion-result.md`](first-completion-result.md)를 우선한다. 공용 H/U ReportPackage 파일의 v1→v1.1 재렌더와 validator 보강은 #48/I2 후속이며, I4·D2-e는 Runtime/recording 후속이다. 이들을 evidence 1차 완료와 혼동하지 않는다.
...
- [x] **사건 유형 불확실과 번호판 ABSTAIN을 구분해 보여준다.** `scenario_unknown_abstain_partial_001`의 `USER_UNSURE`·일반 신고문·WARN 경로와 `scenario_plate_reread_001`의 번호판 부재·`PLATE_REREAD`·Package 미생성 경로를 각각 설명할 수 있다. WARN Package 정책은 v1.1로 종결됐고, 공용 H/U Package fixture 재렌더는 #48/I2 후속으로 분리한다.
...
Research의 권장 스키마·과거 enum·내부 구조 제안은 완료 조건으로 승격하지 않는다. 예를 들어 PDF의 `CONFLICT` 표현보다 [TimeResolution][C1]의 `conflict` 객체와 현재 enum을 따른다. 외부 신고 규정의 최신성을 재조사한 문서가 아니며, 검사 수치의 근거는 채택된 버전의 정책으로 추적한다. Architecture Input Memo는 이번 조건을 정하는 데 추가 사용하지 않았다. 이 문서 최초 작성 시점에는 evidence 공개 실행 코드가 확인되지 않았으나, 현재는 `src/daesingo/evidence` 공개 함수와 baseline runner가 존재한다. 최신 실행 범위는 [`first-completion-result.md`](first-completion-result.md)를 따른다.

Mock Overview·Catalog·Validation Report에는 옛 미해소 설명과 후속 종결 기록이 공존한다. 이 문서는 현재 Final 본문·확정 후속 기록·실제 JSON을 함께 대조했다. 특히 `CorrectionRecord`는 Final v1.1이고, U 시나리오에서 Evidence가 전부 미생성된다는 과거 설명은 현재 기준이 아니다. Q1/Q2 정책 결정은 이후 ADR-EVIDENCE-003과 safety-report-policy/v1.1로 종결됐다. 공용 H/U Package 파일 재렌더는 #48/I2 fixture 후속으로 남는다.
...
- [x] H/U/P/R 각각의 입력, 출력, 예상 상태, 검증 결과를 재현 가능한 실행 기록으로 제시한다. U의 위치 부재·Template 정책은 v1.1 결정을 따르며, 공용 H/U Package fixture의 구버전 파일은 #48/I2 후속으로 별도 추적한다. [M2][C1][C2][C3]
...
- [x] H의 두 scope는 PASS, U의 두 scope는 WARN, P는 UNKNOWN→WARN, R은 WARN→WARN이라는 서로 다른 이유를 설명할 수 있다. P/R은 #97에서 active catalog의 공통 4개 EVIDENCE rule과 공용 fixture·CaseView가 동기화됐고, 위치 부재 WARN을 정책대로 보존한다. 공용 H/U Package fixture의 v1→v1.1 재렌더는 #48/I2의 별도 후속이다. [M2]

### `ReportPackage` — `report-package/v1.1`
...
- [x] 필수 `report_inputs`, 제목·본문·`template_ref`, 신고용 영상 ref, source/derived provenance, `created_at` 및 handoff 정보가 Contract 형식에 맞는다. `location`은 키 필수·값 nullable이며 위치 부재에서는 `null`을 그대로 보존한다. [C3] §7
- [x] 신고문이 확정된 Template와 입력으로 재현되며, 같은 `template_ref`로 서로 다른 임의 문구를 정답 처리하지 않는다. 신고문·Template·policy provenance는 safety-report-policy/v1.1 기준을 따른다. [C3] §8.3[D6]
...
| --- | --- | --- | --- | --- |
| `scenario_happy_001` | `case_h001`/`candidate_h001`, `ve_h001`, 번호판·검증된 overlay, 시각 후보, `clip_h001`, 최종 derived 자산 사실 | 시각·Evidence 확정, EVIDENCE 검사, 자산 준비 후 FINAL_PACKAGE 검사·신고문/Package 조립 | `tres_h001`, `ev_h001`, 빈 items의 Needs, `req_h001_evidence=PASS`, `req_h001_final=PASS`, `pkg_h001` | 시각 `2026-08-24T18:05:12+09:00`, 번호판 `12가3456`과 근거 refs 유지. Consumer가 두 scope와 Package를 읽음. 신고문/정책은 v1.1 baseline으로 검증하며 공용 Package 파일 재렌더는 #48/I2로 분리. |
| `scenario_unknown_abstain_partial_001` | `ve_u001.verification=UNCERTAIN`, 정상 번호판 `88부1234`, overlay `NOT_APPLICABLE`, 충돌하는 파일명/metadata 시각, `USER_UNSURE`, derived 자산 사실 | 시각 fallback과 충돌 보존, 사건 유형 null·사용자 응답 snapshot, 일반 신고문·WARN 판정 | `tres_u001=NEEDS_REVIEW`, `ev_u001`, `req_u001_evidence/final=WARN`, 일반 신고문의 `pkg_u001`이 Catalog의 목표 | 가짜 사건 유형·시각·위치 없음. `safety_report_type.needs_review=true`. 위치 부재 WARN·no-location Template 정책은 v1.1로 종결됐다. 공용 H/U Package 파일의 v1→v1.1 재렌더만 #48/I2 fixture 후속으로 분리한다. |
| `scenario_plate_reread_001` | 관찰된 `SIGNAL`, 검증된 시각, 첫 번호판 `abstained=true`, 이후 `readout_p001_plate_reread` | 확보된 사건·시각 보존, 번호판 보강 Need 반환, 재판독 입력을 받아 새 Evidence/Requirement 산출 | `ev_p001`에 번호판 부재, `PLATE_REREAD`, `req_p001_evidence=UNKNOWN` → `ev_p001_v2.vehicle_number=17나2867`, 새 Needs의 `items=[]`, `req_p001_evidence_v2=WARN` | `selection_rev=1` 유지, 기존 Evidence 보존, 새 plate ref 사용. `case_rev=4`도 `EVIDENCE_REVIEW`; 이 Scenario에는 FINAL_PACKAGE/Package가 없으므로 READY까지 요구하지 않음. |
| `scenario_correction_rerun_001` | 파일명 기반 `tres_r001_v1`/`ev_r001_v1`, case의 `cr_r001_time`(`EVENT_TIME_MANUAL`, `target_field=occurred_at`, `selection_rev=1`) | 사용자 시각을 적용한 새 시간/Evidence 생성, 새 basis로 요건 재평가 | `tres_r001_v1: NEEDS_REVIEW` → `tres_r001_v2: OK/AGREED/USER_OVERRIDE`, `ev_r001_v2`, `req_r001_evidence_v1: WARN` → v2 `WARN` | `13:15:30+09:00` → `13:13:00+09:00`, 번호판 `34나7890`과 나머지 근거 불변, time/evidence/report supersede 연결. 후보 시각·`selection_rev` 불변. Package 생성은 이 Scenario 범위 밖. |
...
- [x] U의 `USER_UNSURE`, null 사건 유형, 충돌 시각, WARN 결과 JSON과 위치 부재/no-location Template v1.1 결정을 함께 제시했다.
...
7. 정책/Contract 결정과 공용 fixture 재렌더 후속을 구분했는가? #48/I2·I4·D2-e를 evidence 1차 완료로 잘못 닫지 않았는가?
...
| evidence → 유소연 `case` → 신유민 `web` | TimeResolution, EvidenceRecord, RequirementReport, ReportPackage / CaseView | authoritative 결과·provenance·검토 필요 전달 | 현재 결과 선택, gate·표시·notice 투영 | 두 scope 보존, USER_UNSURE를 사용자 확정으로 승격하지 않음, Package 유무와 최종 사용자 검토 분리. H/U의 v1.1 위치/Template 정책과 공용 fixture 후속을 구분 |
...
## 현재 상태 / 후속 통합 경계 — 2026-09-20

**evidence의 baseline/Contract/Consumer 접합 1차 완료 조건은 충족했다.** 다만 이것은 실제 장시간 영상 처리나 최종 REPORT_VIDEO 관찰까지 완료됐다는 뜻이 아니다. 남은 작업은 아래처럼 소유 경계를 분리한다.

| 항목 | 현재 상태 | 후속 |
| --- | --- | --- |
| evidence 공개 함수·baseline·Contract 검증 | ✅ 완료 | 현 증빙 유지 |
| P/R 공용 evidence fixture · CaseView | ✅ #97 merge | #86 종결 가능 |
| Happy real E2E 위치 단서 전달 | ✅ #114 merge | GPS Observation은 recording 공개 producer 전까지 알려진 단순화 |
| Web의 `requirements_evidence=WARN` 소비 | ✅ #86에서 확인 | 별도 Web 변경 없음 |
| H/U 공용 ReportPackage v1→v1.1 재렌더·validator | ⏳ evidence 정책은 완료, 공용 파일이 남음 | **#48 / I2**에서 처리 |
| I4 최종 REPORT_VIDEO 번호판·시각 가시성 | ⏳ Runtime Producer 미연결 | #115 기준으로 readout의 REPORT_VIDEO 입력 계약/capability 확장 후 case→evidence 배선 |
| D2-e 녹화 시작·끝 경계 | ⏳ 미결 | recording Owner가 생성 실패/경고/사용자 고지 정책을 별도 결정 |
| common/runtime 실제 Worker·Queue·재시도 등 | 범위 밖 | 해당 Owner 일정에서 처리 |

이 표의 ⏳ 항목을 이유로 이미 입증한 baseline/Contract 체크를 다시 미완료로 돌리지 않는다. 반대로 baseline PASS를 실제 영상 Runtime 완료로 확대해서도 안 된다.
...
[P97]: https://github.com/kakaotechcampus-4/ktc4-chonnam-2/pull/97
[P114]: https://github.com/kakaotechcampus-4/ktc4-chonnam-2/pull/114
[P115]: https://github.com/kakaotechcampus-4/ktc4-chonnam-2/pull/115
