...
| 위치 | 변경 |
| --- | --- |
| §5.6 「위치 — D1 미결과의 관계」 | D1이 ①로 종결됐음을 반영하고 새 revision을 가리킨다 |
| §5.8 렌더 필요 입력 목록 | 「Package 표시용 위치」를 선택된 template 기준으로 읽는다는 단서를 붙인다 |
| §5.11 | 발생장소 「필요」는 유지하되 그 무게를 Package 미발행이 지지 않음을 명시한다 |
| §5.14 남은 미결 목록 | 「D1(Q1) 위치 결론」 항목을 이 ADR로 종결 처리한다 |

K1·K2·K4는 영향을 받지 않는다.
