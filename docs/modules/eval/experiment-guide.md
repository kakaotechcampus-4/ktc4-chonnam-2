...
> 이 표는 이 가이드가 제안한 후보 이름의 전체 목록(11개)이다. 9/7 ADR §4.10 이 「`failure.kind`를 6개로 닫는다」고 확정한 것은 이 중 일부이고, 그 6개는 층위가 둘이다 — **런타임 실패 5종**(실행 중 관측되는 사실)과 **사후 분류 1종(`OVERCONFIDENT`)**. 후자는 GT가 있어야 판정되므로 런타임에는 존재하지 않는다.
