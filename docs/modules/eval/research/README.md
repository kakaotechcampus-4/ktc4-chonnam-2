# eval/research

외부 자료, 비교 조사, 기술 Spike를 둔다. 조사 결과 자체는 결정이 아니다.

- `architecture-input-memo.md` — v4 작성 근거가 된 Owner 자료조사 메모. §10에 2026-09-12 현행 대조 (결정 아님)
- `version-fields-proposal.md` — 예측·결과 파일 version field 제안 원문과 구현 대조 (결정 아님)
- `ground-truth-schema.md` — 평가용 정답지 스키마 자료조사 원문(v0.2, 2026-08-31)과 현행 구현(`gt_version: g1`) 대조 (결정 아님)
- `aihub-71555-survey.md` — A tier 소스(AI-Hub 71555) 사전조사 원문(2026-08-27)과 현행 대조 (결정 아님)
- `eval-dataset-plan.md` — 평가 데이터셋 3-tier(A/B/C) 구성 계획 원문과 현행 대조 (결정 아님)
- `blackbox-storage-survey.md` — 제조사별 블랙박스 저장 방식 메모. **출처 미검증(LLM 응답)** — 인용 금지
