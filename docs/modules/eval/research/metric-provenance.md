...
**아직 문헌보다 약한 것 (F7).** 문헌의 Recall@K 구현은 보통 매칭된 예측을 소비(1:1 배정)한다.
우리는 사건마다 독립으로 훑어서, 한 클립에 사건이 둘이면 둘을 모두 덮는 넓은 예측 하나가
적중 2건으로 센다. 다중 사건 클립이 들어오는 커밋에서 함께 고쳐야 한다(`harness-v1-design.md` §9 F7).
