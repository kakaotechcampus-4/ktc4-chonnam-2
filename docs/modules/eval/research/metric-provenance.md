...
정답지의 사건 나열 순서에 휘둘리지 않게 하기 위해서다. 다만 **순서 무관은 recall 에만 해당한다** —
매칭의 크기는 유일해도 어느 쌍으로 맺는지는 유일하지 않아, 쌍을 쓰는 `onset_error_sec`·
`containment_rate` 는 배정 순서를 따로 고정해야 했다(`harness-v1-design.md` §9 F7).
