...
>
> **2026-09-14 갱신 (Mock Pack v5 반영).** v5(PR [#43](https://github.com/kakaotechcampus-4/ktc4-chonnam-2/pull/43), `develop` @ `72e0e05`)에서 `readout.overlay.ocr_failed` fixture가 **처음 생겼다**(`readout/scenario_infra_failure_001.json`의 `readout_x001_overlay_reread`). 미리 등재해둔 notice 값(`readout.overlay_ocr_failed` · `INFO` · `blocking=false` · `actions=[]`)이 그대로 쓰였음을 4차 재검수(이슈 [#38](https://github.com/kakaotechcampus-4/ktc4-chonnam-2/issues/38))에서 확인했다. 이 fixture 덕분에 세 갈래가 데이터에서 어떻게 갈리는지 관측 가능해져 **「fixture 지문」 표를 신설**했다. 등재값·규칙 변경은 없다.
>
...
### fixture 지문 — 어느 갈래인지 데이터에서 가리는 법 (2026-09-14, Mock Pack v5)

위 표는 **어떤 값을 내야 하는가**를 정하고, 이 표는 **이미 기록된 `OverlayTimeReadout`이 어느 갈래인가**를 가린다. v5에서 `ocr_failed` fixture가 생기면서 네 갈래가 전부 관측 가능해졌다.

| | `status` | `reason.code` | `samples` | `validation.format_ok` | `validation.monotonic_ok` · `duration_match_ok` |
| --- | --- | --- | --- | --- | --- |
| 읽음 (정상) | `OK` | `null` | 2~3건 | `true` | `true` |
| 시각이 안 찍힌 영상 (**사실**) | `NOT_APPLICABLE` | `readout.overlay.not_present` | `[]` | `null` | `null` |
| 있는지 자체를 못 봄 (**모름**) | `UNKNOWN` | `readout.overlay.presence_undetermined` | `[]` | `null` | `null` |
| 읽었으나 못 알아봄 (**모름**) | `UNKNOWN` | `readout.overlay.ocr_failed` | 1건 이상 (`raw_text` 존재, `parsed_at=null`) | `false` | `null` |

읽는 법 두 가지다.

- **`ocr_failed`만 지문으로 갈린다.** 읽으려고 시도한 흔적(`samples[]`)이 남고 `format_ok=false`가 「글자는 잡았는데 시각으로 파싱되지 않았다」를 나른다. 나머지 두 UNKNOWN/NOT_APPLICABLE 갈래는 시도 자체가 없어 `samples=[]` · `validation.*=null`로 **서로 지문이 같다** — 이 둘은 `status`와 `reason.code`로만 갈린다.
- **`format_ok=false`와 `format_ok=null`은 다른 말이다.** `false`는 검사해서 틀렸다는 뜻이고 `null`은 검사할 대상이 없었다는 뜻이다. `monotonic_ok`·`duration_match_ok`가 `ocr_failed`에서도 `null`인 것은 같은 이유다 — 파싱된 시각이 없으니 순서·길이를 비교할 대상이 없다.

`samples` 유무로 `reason.code`를 **역추론하지 않는다.** 위는 기록된 값을 읽는 표이고, 어느 code를 낼지는 아래 판정 기준이 정한다.
...
- ~~`readout.overlay_ocr_failed`는 **등재만 해 둔다.** Mock Pack v4(`d9d8e2b`) 기준으로도 이 갈래 fixture가 없다~~ → **해소 (2026-09-14, Mock Pack v5).** 이슈 [#39](https://github.com/kakaotechcampus-4/ktc4-chonnam-2/issues/39) Non-blocking 후속으로 등록돼 있던 fixture가 v5에 들어왔고(`readout_x001_overlay_reread`), 미리 등재해둔 값을 새로 짓지 않고 그대로 썼다. 세 code 전부 fixture 근거가 있다.
