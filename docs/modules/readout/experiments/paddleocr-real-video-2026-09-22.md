...
| 전체 번호판 detector 후보 (KLPD ONNX) | 최선 `[621.3, 687.2, 101.1, 32.9]` (`xywh`), confidence `0.461`, 수동 전체 bbox 대비 IoU `0.813` (5개 중복 후보) |
| KLPD ONNX, 같은 clip의 14.017초 | 최선 `[619.7, 688.4, 99.3, 31.2]` (`xywh`), confidence `0.343`, 별도 수동 전체 bbox 대비 IoU `0.720` (6개 중복 후보) |
...
- KLPD ONNX 후보는 같은 clip의 두 frame에서 전체 물리 번호판에 가까운 상자를 냈다(최고 IoU `0.813`, `0.720`). 다만 각각 5개·6개의 겹치는 후보를 냈고, 현재는 같은 차량·장면만 수동 라벨했다. 따라서 후보를 즉시 제품 경로에 연결하지 않고, 다른 장면의 수동 라벨 frame으로 일반화와 후보 선택 규칙을 측정한다.
...
현재 PaddleOCR detector는 줄 단위 bbox를 반환하며, 첫 실영상에서 측정한 EasyKorean detector 후보도 전체 plate 경계를 만들지 못했다. KLPD ONNX는 같은 차량의 두 frame에서 유망하지만, 중복 후보 선택과 다른 실제 장면 재현성 측정이 남았다. 그 경로가 준비되기 전에는 하단 줄 bbox를 전체 번호판 bbox로 승격하지 않는다.
