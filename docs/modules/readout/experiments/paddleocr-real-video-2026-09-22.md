...
| 회귀 테스트 | 2026-09-23: `68 passed, 34 subtests passed` |
...
- 황색 2줄 번호판의 하단 줄 `바5215`는 현재 구현에서 여전히 `OK`가 될 수 있는 **미해결 회귀 대상**이다. 전체 번호판 완전성 판단과 `PARTIAL_PLATE_READ` 사유를 계약에 등재한 뒤, `NEEDS_REVIEW`가 되는 회귀 테스트와 구현을 추가한다.
