...
- `intent-hint-robustness-v2.jsonl` — 기존 9카테고리×3(v1 이식) + 기존 9카테고리 추가 변형×2(2026-09-22, 딥리서치 도구 2차 요청 결과 병합, `case-*d`/`case-*e`) + `정보_위치순서_편향` 신규 1카테고리×3 = **48케이스.** v1의 7케이스와 합치면 **55케이스**로 표본 크기 50을 넘겼다. `dataset.py`의 `load_dataset()`으로 무료 로드 확인(2026-09-22) — 파싱 에러 없음, 카테고리당 5개(신규 카테고리만 3개), 중복 id 없음.
- 4개 모델(GPT-5 Nano/Gemini 3.1 Flash-Lite/Claude Haiku 4.5/Gemini 3.8 Flash) 전체 재실측 완료(48케이스, judge_run_id `20260922T124511Z`) — 원본 `results/summary-robustness-v2.md`.
...
## 13. 최종 결정 — 이 문서에 두지 않음

3차례 실측(27→34[+Gemini 3.8 Flash]→48케이스) 전부를 근거로 모델 재선정 최종 결정이 났다. Research는 결정이 아니므로(`docs/README.md`) 결정 내용은 `decisions/intent-llm-model-selection.md` §10~§11에 있다 — 여기서 다시 옮겨 적지 않는다. 요약만: **GPT-5 Nano 채택 철회, Gemini 3.8 Flash로 교체.**
