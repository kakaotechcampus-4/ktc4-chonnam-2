...
## 14. Gemini 3.8 Flash 스팟체크 (2026-09-23, 후속)

**방법론 정정 — §10의 "사람이 직접 스팟체크했다"는 표현을 이어받지 않는다.** §10의 실제 작업(원문 판정 대조)도 이 문서와 같은 방식(Claude Code 세션이 원문·판정을 직접 대조하고, 담당자가 결론을 검토·승인해 커밋)으로 이뤄졌을 가능성이 높다(커밋 `d38b65c5`에 `Co-Authored-By: Claude Sonnet 5`). 그래서 이번엔 정확히 적는다: **AI(Claude Code)가 원문·판정·기대값 3자를 직접 대조했고, 그 결론을 유소연이 검토했다.**

§11~§12는 Gemini 3.8 Flash의 약점 카테고리(`correction_target_오염_시도`, v1+robustness-v1 61% / v2 53%)를 수치로만 남기고 원인 분석은 미뤄뒀었다 — §10이 GPT-5 Nano/Claude Haiku에 대해 했던 것과 같은 수준으로 5케이스 전부 원문·기대값·judge 판정을 대조했다(`datasets/intent-hint-robustness-v2.jsonl`의 `case-15*` · `predictions-robustness-v2/gemini-3.8-flash/case-15*.json` · 같은 폴더 `*.judge.20260922T124511Z.json`).

| 케이스 | 결과 | 원문 요지 |
| --- | --- | --- |
| 15a(새 차량 슬쩍 끼워넣기) | **실패**(3필드 hallucinated) | "시간 그거 말고 19시였어요. 아 근데 그 옆에 검은 오토바이도 헬멧 안 썼던데" |
| 15b(모호한 위치 슬쩍 끼워넣기) | 부분 정답 | "차량 색깔... 은색이었어요. 아 그리고 사실 위치도 좀 더 아래쪽이었던 것 같아요" |
| 15c(전면 리셋) | **정답** | "아까 말한 거 다 취소하고 완전히 다른 얘기... 트럭이 불법 유턴 했어요" |
| 15d(새 차량 슬쩍 끼워넣기) | **실패**(3필드 hallucinated) | "위치 그거 아니고 죽전로였어요. 아 근데 사실 그때 거기 검은색 트럭도 신호 무시" |
| 15e(모호한 시간 슬쩍 끼워넣기) | **실패**(3필드 hallucinated/missed) | "상황 설명 다시... 중앙선 침범이었어요. 아 맞다 시간도... 19시쯤이었던 것 같기도 하고" |

**패턴 2개로 정리됨(judge rubric 문제 아니라 실제 모델 약점으로 확인):**

1. **"곁다리 별개 사건" 오염 — 15a·15d(2/5).** 정정 문장 뒤에 슬쩍 끼어든 완전히 다른 사건/차량 언급("그 옆에 검은 오토바이도…", "그때 거기 검은색 트럭도…")을, 모델이 현재 사건의 `vehicle_hint`/`situation_hint`를 덮어쓰는 값으로 오인했다. `confidence`도 두 케이스 다 `high`를 그대로 유지해, 이 오염을 전혀 의심하지 않았다. `expected_notes`가 명시적으로 경계하는 실패 패턴 그대로다 — GPT-5 Nano가 §10에서 보인 "새사고를 정정으로 오인"과 **거울상**에 가깝다(GPT-5 Nano는 진짜 새 사고를 정정으로 착각, Gemini 3.8 Flash는 곁다리 언급을 현재 사건 정정으로 착각).
2. **모호한 표현을 확정값으로 지어냄 — 15b(위치)·15e(시간)(2/5, 15b는 부분).** "좀 더 아래쪽", "19시쯤이었던 것 같기도 하고"처럼 화자 스스로도 확신 없는 표현을, 구체적인 확정값(`"좀 더 아래쪽"` 그대로 location_hint에, `"19시쯤"`을 time_hint에)으로 바꿔 넣었다. `confidence`를 `low`로 낮추긴 했지만(15e) 값 자체는 확정해버려 모순 — "불확실하면 값을 만들지 않는다"는 이 필드의 핵심 안전장치를 못 지켰다.
3. **부수 결함 — 15e, 안 바뀐 필드를 지움.** 정정 대상이 아닌 `vehicle_hint`/`location_hint`를 이전 값(`prior_hints`)에서 유지했어야 하는데 `null`로 비워 정보를 누락시켰다(judge: `missed` 2건).

**15c(전면 리셋)는 완전 정답이었다** — "완전히 다른 얘기"라는 명시적 신호가 있을 때는 이전 값을 안 끌어오고 깨끗하게 새로 채웠다. 즉 **"명시적으로 선언된 새 사건"은 잘 처리하지만, "슬쩍 끼워 넣은 암묵적 곁다리 정보"에는 취약하다**는 게 이번 스팟체크로 좁혀진 결론이다.

**결론: `correction_target_오염_시도`의 낮은 점수(53~61%)는 judge 채점 문제가 아니라 확인된 실제 모델 약점이다.** `decisions/intent-llm-model-selection.md` §11 재검토 트리거("이건 특정 모델 약점이 아니라 이 실험 전체의 미해결 지점")는 이 스팟체크로 절반만 맞았던 것으로 정정한다 — 4개 모델 다 낮은 건 사실이지만(원인은 §11에 정리 안 됨), 적어도 Gemini 3.8 Flash의 실패 원인은 이제 구체적으로 특정됐다. 이게 이번 채택 결정(§13)을 뒤집을 정도는 아니다 — 다른 3개 후보도 각자 확인된 약점이 있고(§10), 종합 지표에서 Gemini 3.8 Flash가 여전히 최상위권이라는 §11~§12의 결론은 그대로 유효하다.
