...
**Accepted:** `2026-09-05`
**수락 근거:** `common/runtime`은 PM 단독 소유이고, 값의 대부분은 `adr-job-record-case-view.md` 부록-A가 3인 리뷰를 거쳐 이미 확정한 것이다(status 5값 · `CANCELLED` 미포함 · 캐시 semantics). PM이 새로 정한 3건(§10)은 모두 **필드 추가**이며 소비자의 기존 소비를 제약하지 않는다. 소비자 이견이 오면 개정 ADR로 처리한다
...
> **이 문서가 왜 지금 생겼나.** `JobRecord` ADR이 실행 상태(status/attempt/cost/produced/failure_kind)를 `JobRecord`에서 떼어내 별도 `JobExecution` 계약으로 이관하기로 확정했는데(부록-A §6·§10·§12), 그 계약 문서가 없었다. 목데이터 통합에서 queue 목 응답을 만들 근거가 없으므로 PM이 **ADR이 이미 고정한 값만 모아** 작성했다. 새로 정한 것은 §10에 따로 표시했고, 정하지 않은 것은 §11에 미결로 남겼다.
...
| `produced` | ContractRef[] | Y(빈 배열 허용) | 이 실행이 만든 산출물 참조 | ADR 부록-A §6 이관 목록 |
...
### `CaseView`로의 projection — 확정

`CaseView` 계약(부록-B §10)이 고정한 매핑이다. **본 계약은 이 매핑을 소유하지 않고 따른다.**
...
6. **`JobRecord.force_rerun=false`이고 `input_fingerprint`가 동일한 기존 `SUCCEEDED` 실행이 있으면 새 실행을 만들지 않고 그 결과를 재사용한다. `FAILED`/`STALE`은 cache hit가 아니다** (ADR 부록-A §7·§10-2).
...
ADR이 값을 고정하지 않았고 목 응답을 만들려면 필요해서 PM이 정한 항목이다. **소비자는 여기만 보면 된다.** 셋 다 필드 추가라 기존 소비를 깨지 않는다.
