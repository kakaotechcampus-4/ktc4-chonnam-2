...
- 월요일 Baseline에서 Case는 실제 `register_local_source(path)` 반환값의 `RegisteredSource.media_streams`와 `AnalysisSource.media_stream_refs[]`를 매칭해 `search.AnalysisSourceStream(media_stream_ref, media_type)` 실행 입력을 만든다. 실제 데이터 경로에서 `fixture.media_streams`로 대체하지 않는다.
- Search는 `verify_visual_with_stream_context(...)`의 별도 실행 문맥으로 실제 분석에 사용한 VIDEO를 반환한다. Case는 `execution.result`를 canonical `VisualVerificationResult`로, `execution.selected_video_stream.media_stream_ref`를 후속 Recording 호출 입력으로 사용한다. `CandidateEvent`와 `VisualVerificationResult` canonical 직렬화에는 실행 문맥 필드를 추가하지 않는다.
...
- 위 전달 방식은 월요일 실행 문맥 합의이며 canonical `CandidateEvent`·`VisualVerificationResult`·`SpanResolution` 구조를 바꾸지 않는다. `stream_selector`의 정확한 직렬화와 기본 선택 정책은 W7 후속으로 남긴다.
...
### OI-3. 월요일 VIDEO stream 전달 경계 — API 모양 확정, Case 배선 진행 중

**상태:** 정철원(recording)·서어진(search)·유소연(case) 책임 합의 완료. Search가 별도 실행 문맥 API를 구현했으며, Case가 이 모양에 맞춰 통합 실행기를 배선한다.

```python
execution = search.verify_visual_with_stream_context(
    input_ref=analysis_source_ref,
    candidate=candidate,
    analysis_source_streams=analysis_source_streams,
    target_hint=scope.hint,
)
visual_result = execution.result
media_stream_ref = execution.selected_video_stream.media_stream_ref
```

- `analysis_source_streams`는 `search.AnalysisSourceStream(media_stream_ref, media_type)`의 sequence다. 월요일 실제 데이터 경로에서는 `register_local_source(path)`가 반환한 실제 `MediaStream` 사실을 `AnalysisSource.media_stream_refs[]`와 매칭해 만든다. fixture의 `media_streams`는 fixture 테스트에만 사용할 수 있다.
- Search는 명시된 `media_type` 기준 VIDEO가 정확히 하나일 때만 선택한다. 첫 stream 자동 선택이나 opaque ref 이름 기반 추론을 하지 않으며, VIDEO가 없거나 여러 개이면 `VideoStreamSelectionError`로 실패한다. AUDIO는 제외한다.
- Search는 실제 사용 VIDEO `media_stream_ref`를 별도 실행 문맥으로 Case에 제공한다. `CandidateEvent`와 `VisualVerificationResult` canonical 직렬화는 변경하지 않는다.
...
따라서 Recording의 월요일 blocker는 선택 정책 결정이 아니라, 합의된 명시적 `media_stream_ref`를 받는 공개 capability와 실제 span 계산을 구현·검증하는 것이다. 동일 프로세스의 월요일 runner는 `RegisteredSource.media_streams`를 유지해 연결할 수 있다. 향후 `analysis_source_ref`만 가진 별도 프로세스에서 MediaStream metadata를 다시 얻는 공개 lookup capability는 후속 접합으로 남기며, repository 직접 접근이나 fixture fallback으로 우회하지 않는다.
...
- [x] Search→Case 별도 실행 문맥의 필드 모양과 `verify_visual_with_stream_context()` 함수 시그니처가 구현됐다.
- [ ] Case가 실제 `RegisteredSource.media_streams`와 `AnalysisSource.media_stream_refs[]`를 매칭해 `AnalysisSourceStream[]`을 구성한다.
...
- [ ] Case가 실제 MediaStream 사실로 만든 `AnalysisSourceStream[]`을 `verify_visual_with_stream_context()`에 전달하고, Search가 반환한 VIDEO `media_stream_ref`를 실제 candidate의 timeline/revision/range와 함께 Recording에 전달한다.
