...
§1의 **10·11·13·14번은 검수 대상에서 빼는 것이 아니라, 적용 시점을 Pre-deploy로 미룬 항목**이다. 이 문서는 새로운 Privacy 정책 원문을 만들지 않고 기존 Product/Architecture 경계를 언제 다시 확인할지만 관리한다.

현재 Mock·합성 데이터·팀 내부 개발 단계에서는 전역 masking logger나 PII sanitizer를 강제로 적용하지 않는다. 테스트 fixture와 내부 디버깅 로그의 관찰 가능성을 불필요하게 떨어뜨리지 않기 위해서다. 따라서 이 단계에서는 10·11·13·14번의 확인일·판정 칸을 비워 둔다.

단, 다음 원칙은 개발 중에도 유지한다.

- 실제 사용자 영상·실제 차량번호·정확한 좌표·원본 frame·외부 provider payload 전문·사용자 free text 전문을 **필요 없이 새 운영 로그에 추가하지 않는다.**
- synthetic/mock fixture의 번호판·좌표·문장은 디버깅을 위해 그대로 사용할 수 있다.
- `AnalysisScope.hint.free_text`는 실제 사용자 입력을 외부 provider에 전달하기 전까지 Mock 입력으로만 다룰 수 있다.
- 실제 사용자 `free_text`를 외부 provider에 전달해야 한다면, 먼저 **raw free text 자체를 보내지 않고 구조화된 최소 힌트만 전달할 수 있는지** 검토한다. 불가능할 때 PII sanitization을 구현·검증한다.

아래 중 하나라도 발생하면 10·11·13·14번을 다시 열고 실제 코드 기준으로 PASS/WARN/BLOCK을 판정한다.

1. 실제 사용자 데이터로 통합 테스트를 시작한다.
2. 실제 사용자 입력이나 영상이 Gemini 등 외부 AI provider로 전송되기 시작한다.
3. 외부 공개 데모 또는 Production 배포를 준비한다.
4. §5의 배포 전 전체 리뷰를 수행한다.

이 Deferred 처리는 **배포 면제나 PASS 선처리가 아니다.** 재검토 시점까지 구현을 강제하지 않는다는 일정 결정일 뿐이며, §5 전체 리뷰를 닫으려면 최종적으로 10·11·13·14번도 확인일과 판정을 가져야 한다.
