...
§1의 **10·11·13·14번은 현재 개발 작업의 구현 요구사항이나 리뷰 차단 조건으로 사용하지 않는다.** Mock·합성 데이터·팀 내부 개발 단계에서는 테스트와 디버깅 편의를 우선하고, 전역 masking logger나 PII sanitizer를 별도 선행조건으로 두지 않는다.

따라서 개발 중에는 이 항목 때문에 추가 구현·테스트·리팩터링을 만들 필요가 없으며, AI/자동화 도구도 이 섹션을 근거로 현재 작업 범위를 확장하지 않는다. synthetic/mock fixture의 번호판·좌표·문장 등은 필요한 범위에서 그대로 사용할 수 있다.

Privacy 관련 세부 처리는 개발 마무리 단계에서 다시 본다. 실제 사용자 데이터나 외부 provider 연동이 본격화되면 그 시점의 구조를 기준으로 raw `free_text` 전달 여부, logging 범위, 필요한 sanitization 수준을 결정한다.

재검토 시점:

1. 외부 공개 데모 또는 Production 배포를 준비할 때
2. §5의 배포 전 전체 리뷰를 수행할 때
3. 실제 사용자 데이터/외부 provider 연동이 개발 범위를 넘어 운영 수준으로 들어갈 때

이 Deferred 처리는 Privacy 요구사항을 없애는 결정은 아니다. 다만 **개발 중에는 별도 준수 작업으로 다루지 않고**, 최종 Pre-deploy review에서 실제 코드와 운영 경계를 기준으로 10·11·13·14번을 판정한다.
